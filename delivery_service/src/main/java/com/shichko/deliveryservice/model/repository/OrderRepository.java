...
public interface OrderRepository extends CommonRepository<Order> {
    ...
    @Query("select o from orders o where o.courier is null and not o.state = 'Canceled' order by o.preferredRangeEnd")
    List<Order> getAvailableOrders();

    @Query("select o from orders o where o.courier.id = :courierId and not o.state = 'Delivered' and not o.state = 'Canceled'")
